      ...
      <CardContent className="p-0">
        ...
        <div className="relative">
          <div className="relative h-auto w-full bg-gray-100">
            ...
            {/* Play button overlay */}
            <div
              className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 opacity-0 transition-opacity hover:opacity-100 cursor-pointer"
              onClick={() => {
                router.push(`/courses/watch/${courseId}/${video.id}?vid=${video.vimeoId}`)
              }}
            >
            ...
            <div className="absolute top-4 right-4">
              <Badge variant="outline" className={`${getStatusColor(status)} shadow-sm`}>
        ...
        <div className="p-6">
          <div className="mb-3">
            <h3 className="mb-2 text-lg font-medium text-gray-900">{video.title}</h3>
            <p className="text-sm text-gray-600 line-clamp-2">{video.description}</p>
          </div>

          {video.lastWatched && (
            <div className="mb-3 text-sm text-gray-500">Last watched: {formatDate(video.lastWatched)}</div>
          )}
          ...
          {video.progress > 0 && (
            <div className="mb-4 space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-gray-700">{video.progress}%</span>
              </div>
